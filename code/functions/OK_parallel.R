...
OK_parallel <- function(vgm.model, locn_shp, newdata_shp, maxdist.multiplier = 0.5, force.option = T, 
                        nmin = NULL, nmax = NULL, dependent.var, debuglev = -1, parallel = T, n_cores = NA){
  ...
  maxdist = max(vgm.model$exp_var$dist)
  ...
  if(parallel == T){
    ...
    parallelKrige <- foreach(X = 1:length(parts), 
                             ...
                             .export = c("locn_shp","newdata_shp","maxdist",
                                                              ...
                                                              "nmin", "nmax", "form", "debuglev", "n_cores")) %dopar% {
                                                                gstat::krige(formula =  form, # ordinary kriging
                                                                             ...
                                                                             maxdist = maxdist*maxdist.multiplier, # specify maximum distance to speed up the kriging process
                                                                             nmin = nmin,
                                                                             nmax = nmax,
